...
Загружаем пакеты: `RPostgres` - не используется, `plotly`, `ggplot2`
...
```{r}
slice_df <- df[df$gameId==2018090600,]
slice_df <- slice_df[slice_df$playId == 2474,]
play_moment <- plays_df[plays_df$gameId==2018090600 & plays_df$playId == 2474,]
...
plt <- ggplot(slice_df, aes(frame = frameId)) + 
  ...
  geom_vline(xintercept = 10 + play_moment$yardlineNumber, colour='red') +
